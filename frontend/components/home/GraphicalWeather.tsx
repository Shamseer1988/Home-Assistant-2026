...
function WeatherArt({ condition, size = 1.5 }: { condition: string; size?: number }) {
  const c = (condition || "").toLowerCase();
  const rainy = /rain|pour|drizzle|storm|lightning/.test(c);
  const cloudy = /cloud|overcast|fog/.test(c);
  return (
    <svg viewBox="0 0 130 96" className="h-24 w-28 shrink-0" style={{ transform: `scale(${size / 1.5})` }}>
      {!cloudy && (
      ...
      {rainy && (
        <g className="weather-rain" stroke="#93c5fd" strokeWidth="2.5" strokeLinecap="round">
...
export function GraphicalWeather() {
  ...
  return (
    <div
      className="relative overflow-hidden rounded-3xl border border-white/10 p-5 text-white shadow-glass"
      style={{ background: "linear-gradient(135deg,#1e3a96 0%,#10204d 100%)" }}
    >
      ...
      <div className="mt-3 flex gap-2">
        {a.humidity != null && (
          <span className="flex items-center gap-1 rounded-full bg-white/10 px-2.5 py-1 text-xs">
            <Droplets className="h-3.5 w-3.5 text-blue-200" /> {a.humidity}%
          </span>
        )}
        {a.wind_speed != null && (
          <span className="flex items-center gap-1 rounded-full bg-white/10 px-2.5 py-1 text-xs">
            <Wind className="h-3.5 w-3.5 text-blue-200" /> {a.wind_speed} {a.wind_speed_unit || "km/h"}
          </span>
        )}
      </div>
      ...
      <Link href="/weather" className="mt-3 block text-center text-xs text-blue-200 hover:underline">
        Full forecast →
      </Link>
    </div>
  );
